...
def get_label_command(command, label):
    if command == 'label':
        return ['(' + label + ')']
...
def get_push_with_address_pointer(segment, i):
    return ["@" + i, "D=A", "@"+segment, "A=M+D", "D=M", "@SP", "A=M", "M=D", "@SP", "M=M+1"]


def get_push_with_direct_address(segment):
    return ["@"+segment, "D=M", "@SP", "A=M", "M=D", "@SP", "M=M+1"]


def get_pop_with_address_pointer(segment, i):
    return ["@"+i, "D=A", "@"+segment, "M=M+D //segment=base+i",
            "@SP", "M=M-1", "A=M", "D=M", "@"+segment, "A=M", "M=D", "@"+i, "D=A",
            "@"+segment, "M=M-D //segment=base+i-i"]


def get_pop_with_direct_address(segment):
    return["@SP", "M=M-1", "A=M", "D=M", "@"+segment, "M=D"]




def get_add():
    return["@SP", "M=M-1", "A=M", "D=M  //D=y", "A=A-1", "M=M+D //x=x+y"]
