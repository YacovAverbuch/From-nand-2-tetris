...
def create_output(list_of_xml_lists):
    ...
    for file_list in list_of_xml_lists:
        xml_file_name = file_list[0][:-4] + 'xml'
...
def all_files_to_xml(path):
    """
    Iterate over all the jack files in path (or the only file if path is a file)
    translate every file into a list of xml strings. then call the function to create the output
    ...
    for file_name in files_list:
        ...
        list_of_xml_files.append((file_name, analyzer.XML_lines))
    create_output(list_of_xml_files)
    return


if __name__ == "__main__":
    all_files_to_xml(sys.argv[1])
